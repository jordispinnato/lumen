export async function sendAppointmentConfirmationEmail({ to, patientName, specialistName, slotDate, slotTime }) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject: "Confirmación de turno - LUMEN",
      html: `
        <div style="font-family: Arial, sans-serif; color: #143538; line-height: 1.6;">
          <h1 style="margin-bottom: 8px;">Tu turno está confirmado</h1>
          ...
          <p>Más adelante vas a recibir por este mismo medio los detalles de acceso a la consulta.</p>
          <p>Equipo LUMEN</p>
        </div>
      `,
    }),
  ...
  if (!response.ok) {
    ...
    throw new Error(text || "No se pudo enviar el email de confirmación");
  }
    ...
    body: JSON.stringify({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; color: #143538; line-height: 1.6;">
          ...
          <p>Hola${specialistName ? ` ${specialistName}` : ""}, tenés una nueva reserva en LUMEN.</p>
          <div style="border: 1px solid #d9e2de; border-radius: 10px; padding: 18px; margin: 20px 0;">
          ...
          <p>También podés revisar tus turnos desde tu panel de especialista.</p>
          <p>Equipo LUMEN</p>
        </div>
      `,
    }),
