...
export async function sendContactMessageNotificationEmail({ firstName, lastName, email, phone, subject, message }) {
  const to = process.env.CONTACT_EMAIL || process.env.EMAIL_REPLY_TO || process.env.EMAIL_FROM;

  if (!process.env.RESEND_API_KEY || !process.env.EMAIL_FROM || !to) {
    return { skipped: true };
  }

  const fullName = [firstName, lastName].filter(Boolean).join(" ") || "Persona sin nombre";
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject: `Nueva consulta web - ${subject || "LUMEN"}`,
      html: `
        <div style="font-family: Arial, sans-serif; color: #143538; line-height: 1.6;">
          <h1 style="margin-bottom: 8px;">Nueva consulta desde la web</h1>
          <p>Una persona envio una consulta desde el formulario de contacto de LUMEN.</p>
          <div style="border: 1px solid #d9e2de; border-radius: 10px; padding: 18px; margin: 20px 0;">
            ...
            <p><strong>Telefono:</strong> ${phone || "Sin telefono"}</p>
            <p><strong>Asunto:</strong> ${subject || "Consulta general"}</p>
            <p><strong>Consulta:</strong></p>
            <p>${String(message || "").replace(/\n/g, "<br />")}</p>
          </div>
          <p>Tambien podes verla desde el panel admin, seccion Contacto.</p>
          <p>Equipo LUMEN</p>
        </div>
      `,
    }),
  ...
  if (!response.ok) {
    ...
    throw new Error(text || "No se pudo enviar el email de contacto");
  }
