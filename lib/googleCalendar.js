...
async function getValidAccessToken(supabase, connection) {
...
export async function updateGoogleCalendarEvent({ supabase, specialist, connection, eventId, slot, booking }) {
  if (!eventId) {
    return { skipped: true };
  }
  ...
  const response = await fetch(
    `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events/${encodeURIComponent(eventId)}?sendUpdates=all`,
    {
      ...
      body: JSON.stringify({
        summary: `LUMEN - Consulta con ${patientLabel}`,
        description: [
          "Turno reprogramado desde LUMEN.",
          `Paciente: ${patientLabel}`,
          booking.patient_email ? `Email: ${booking.patient_email}` : "",
        ].filter(Boolean).join("\n"),
        start: {
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(text || "No se pudo actualizar el evento en Google Calendar");
  }

  return { updated: true, event: await response.json() };
}

export async function deleteGoogleCalendarEvent({ supabase, connection, eventId }) {
  if (!eventId) {
    return { skipped: true };
  }
  ...
  // Google Calendar devuelve 404/410 si el evento ya no existe (por ejemplo, si el
  // especialista lo borro a mano) - lo tratamos como exito, no como error.
  if (!response.ok && response.status !== 404 && response.status !== 410) {
    const text = await response.text();
    throw new Error(text || "No se pudo cancelar el evento en Google Calendar");
  }
